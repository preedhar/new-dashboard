...
function defaultPayoutDetails(): PayoutDetails {
  return { bankName: '', accountName: '', accountNumber: '' }
}

// The payout details dialog, opened from the Automated payments menu.
function PayoutDetailsDialog({
  ...
  const [draft, setDraft] = React.useState<PayoutDetails>(details)

  function update<K extends keyof PayoutDetails>(key: K, value: PayoutDetails[K]) {
  ...
  const hasChanges = JSON.stringify(payload) !== JSON.stringify(details)
  const canSave =
    hasChanges && payload.accountNumber !== '' && payload.bankName !== ''
    ...
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[calc(100svh-2rem)] flex-col gap-6 overflow-hidden sm:max-w-lg [&_[data-slot=dialog-close]]:size-10">
        <DialogHeader className="shrink-0">
          <DialogTitle asChild>
            <TypographyH4 className="font-semibold">Payout details</TypographyH4>
          </DialogTitle>
        </DialogHeader>

        <div className="-mx-6 flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto px-6">
          <div className="space-y-1.5">
            <Label htmlFor="payout-bank-name" className="text-sm font-medium">
              Bank name
            </Label>
            <Input
              id="payout-bank-name"
        ...
        <div className="-mx-6 flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto px-6">
          ...
          <div className="space-y-1.5">
            ...
            <ToggleGroup
              ...
              onValueChange={(value) =>
                update('channels', value as ChargeChannel[])
              }
              className="w-full"
            >
              {CHARGE_CHANNELS.map((channel) => (
                <ToggleGroupItem
                  ...
                  className="group/channel gap-2 px-2 font-normal data-[state=on]:bg-background data-[state=off]:border-border/50 data-[state=off]:bg-muted data-[state=off]:text-muted-foreground"
                >
                  <img
                    src={channel.icon}
                    alt=""
...
export function AdminSettingsPaymentsPage() {
  ...
  const [payout, setPayout] = React.useState<PayoutDetails>(defaultPayoutDetails)
  ...
  function savePayout(details: PayoutDetails) {
    setPayout(details)
    setPayoutOpen(false)
    toast.success('Changes saved')
  }
      ...
      <div className="w-full">
        ...
        <div className="mx-auto flex w-full max-w-[640px] flex-col gap-8">
          <PaymentSection
            ...
            description={
              <>
                Orders get approved after payments are automatically verified.{' '}
                <a
            ...
            <SettingsCard>
              ...
              >
                <Switch
                  ...
                  checked={automated.card}
                  onCheckedChange={(checked) => toggle('card', checked)}
                />
              </SettingRow>
              <SettingRow
                id="pay-paynow"
                label="PayNow"
                icon={PayNowIcon}
                description="Customers pay by scanning a PayNow QR code"
              >
                <Switch
                  id="pay-paynow"
                  aria-label="PayNow"
                  checked={automated.payNow}
                  onCheckedChange={(checked) => toggle('payNow', checked)}
                />
              </SettingRow>
              <SettingRow
                id="pay-paypal"
                label="PayPal"
                icon={PayPalIcon}
                description="Let customers check out with their PayPal account"
              >
                <Switch
                  id="pay-paypal"
                  aria-label="PayPal"
                  checked={automated.payPal}
                  onCheckedChange={(checked) => toggle('payPal', checked)}
                />
              </SettingRow>
            </SettingsCard>
              ...
              <SettingRow
                ...
                description={
                  <span className="inline-flex items-center gap-1.5">
                    Add processing fees to the order total at checkout
                    <TooltipProvider>
                      <Tooltip>
          ...
          <PaymentSection title="More">
            <SettingsCard>
              <NavSettingRow
                ...
                icon={HandCoins}
                description="Manually verify each payment and approve orders"
            ...
            <SettingsCard>
              <NavSettingRow
                label="Tips"
                icon={Coins}
                description="Let customers select a tip during checkout"
            ...
            <SettingsCard>
              <SwitchNavRow
                label="Charges"
                icon={Receipt}
                description="Add service charges to orders"
            ...
            <SettingsCard>
              <SwitchNavRow
                label="Tax"
                icon={Percent}
                description="Collect tax on your orders"
                checked={tax.enabled}
    ...
    fields: [
      {
        key: 'account',
        label: 'Account',
        placeholder: 'Mobile number or UEN',
      },
      INSTRUCTIONS_FIELD,
    ],
  },
...
function defaultMethodState(): Record<DefaultMethodId, DefaultMethodState> {
  return {
    paynow: { enabled: false, values: {} },
    paypal: { enabled: false, values: {} },
    'bank-transfer': { enabled: false, values: {} },
    cash: { enabled: false, values: {} },
  }
}

// Summary shown under a default method's name: its key detail when configured,
// otherwise its on/off state.
function defaultMethodSummary(
  ...
  const value = state.values[primary.key]?.trim()
  if (value && !primary.multiline) return value
  return 'On'
}
      ...
      <DialogContent className="flex max-h-[calc(100svh-2rem)] flex-col gap-6 overflow-hidden sm:max-w-lg [&_[data-slot=dialog-close]]:size-10">
        ...
        <div className="-mx-6 flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto px-6">
          <div className="flex items-center justify-between gap-4">
            <Label htmlFor="default-method-enabled" className="text-sm font-medium">
              Accept {config.name}
            </Label>
            <Switch
              id="default-method-enabled"
...
// The add/edit dialog for a custom method: a name and free-form details.
// Re-mounted per open so its draft always starts fresh from `initial`.
function CustomMethodDialog({
  ...
  onOpenChange: (open: boolean) => void
  onSave: (method: { name: string; details: string }) => void
}) {
  const isEditing = initial !== null
  const [name, setName] = React.useState(initial?.name ?? '')
  const [details, setDetails] = React.useState(initial?.details ?? '')

  const payload = { name: name.trim(), details: details.trim() }
  const hasChanges =
    !isEditing ||
...
export function AdminManualPaymentsPage() {
  ...
  function saveDefault(id: DefaultMethodId, state: DefaultMethodState) {
    setDefaults((current) => ({ ...current, [id]: state }))
    setOpenDefault(null)
    toast.success('Changes saved')
  }

  function saveCustomMethod(draft: { name: string; details: string }) {
      ...
      <div className="w-full">
        ...
        <div className="mx-auto flex w-full max-w-[640px] flex-col gap-8">
          <SettingsCard>
            {DEFAULT_METHODS.map((config) => (
              <NavSettingRow
                ...
                onClick={() => setOpenDefault(config.id)}
              />
            ))}
          </SettingsCard>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <TypographyLarge>Custom methods</TypographyLarge>
              <Button
                type="button"
                variant="outline"
