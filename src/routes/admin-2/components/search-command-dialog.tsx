...
type CommandNavItem = {
  ...
  url: string
  parentTitle?: string
  icon: Admin2NavItem["icon"]
}

const navByTitle = new Map(
  [...primaryAdmin2Nav, ...appsAdmin2Nav].flatMap((item) => [
  ...
  },
  {
    ...
    items: [
      navByTitle.get("POS"),
      {
        ...
        url: "/admin-2/marketing/loyalty",
        parentTitle: "Marketing",
        icon: primaryAdmin2Nav.find((item) => item.title === "Marketing")!.icon,
      },
      {
        title: "Kitchen Display",
        url: "/admin-2/apps",
        parentTitle: "Apps",
        icon: appsAdmin2Nav.find((item) => item.title === "All Apps")!.icon,
      },
    ].filter((item): item is CommandNavItem => Boolean(item)),
  },
]
